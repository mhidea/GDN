...
class MSTGATLstm(BaseModel):
    """docstring for MSTGATLstm."""

    def __init__(self, **kwargs):
        super(MSTGATLstm, self).__init__(**kwargs)
        self.gamma1 = kwargs["gamma1"]
        self.gamma2 = kwargs["gamma2"]
        kernel_size = kwargs["kernel_size"]
        ...
        # Linear layer for each node (sensor)
        self.lin_predictor = MLP(
            in_channels=self.param.embedding_dimension * 2, out_channels=1, num_layers=1
        )
        self.vae = MyVGAE(
            (
                VariationalGCNEncoder(
                    self.param.embedding_dimension * 2,
                    out_channels=self.param.out_layer_inter_dim,
                )
            )
        )
...
class MSTGAT3(BaseModel):
    """docstring for MSTGAT."""

    def __init__(self, **kwargs):
        super(MSTGAT3, self).__init__(**kwargs)
        self.gamma1 = kwargs["gamma1"]
        self.gamma2 = kwargs["gamma2"]
        kernel_size = kwargs["kernel_size"]
